...
def _handle_trakt_continue_watching(args: argparse.Namespace) -> None:
    ...
    try:
        structured = ContinueWatchingPayload.model_validate(payload)
    except ValidationError as exc:
        exit_with_error(f"Unable to parse continue watching payload: {exc}")
        return

    _print_continue_watching(structured)
...
def _print_continue_watching(payload: ContinueWatchingPayload) -> None:
    movies = list(payload.movies or [])
    shows = list(payload.shows or [])
    ...
    if not movies:
        ...
        for movie in movies:
            ...
            title = getattr(media, "title", None) or "Unknown Title"
            progress_value = getattr(movie, "progress", None)
            ...
            resume_suffix = " (resume available)" if getattr(movie, "resume_available", False) else ""
            print(f"  - {title} — Progress: {progress_text}{resume_suffix}")

            last_watched = getattr(movie, "paused_at", None)
            last_watched_text = _format_timestamp(last_watched)
            if last_watched_text:
                print(f"    Last watched: {last_watched_text}")

            playback_id = getattr(movie, "playback_id", None)
            if playback_id is not None and getattr(movie, "resume_available", False):
                print(f"    Resume playback ID: {playback_id}")
    ...
    for show in shows:
        show_media = getattr(show, "show", None)
        show_title = getattr(show_media, "title", None) or "Unknown Show"
        next_episode = getattr(show, "next_episode", None)
        tracked_episodes = list(getattr(show, "episodes", []) or [])

        if next_episode is None:
            tracked_count = len(tracked_episodes)
            if tracked_count:
                plural = "episode" if tracked_count == 1 else "episodes"
                print(f"  - {show_title} — Up to date (tracking {tracked_count} {plural})")
            else:
                print(f"  - {show_title} — Up to date")
            continue

        episode_code = _format_episode_code(getattr(next_episode, "season", None), getattr(next_episode, "number", None))
        episode_media = getattr(next_episode, "episode", None)
        episode_title = getattr(episode_media, "title", None) or "Unknown Episode"

        progress_value = getattr(next_episode, "progress", None)
        progress_text = _format_progress(progress_value, completed=getattr(next_episode, "completed", False))
        resume_suffix = " (resume available)" if getattr(next_episode, "resume_available", False) else ""
        remaining = _format_remaining(getattr(show, "unwatched_count", None))

        details = [f"Next {episode_code}: {episode_title}", f"Progress: {progress_text}{resume_suffix}"]
        if remaining:
            details.append(remaining)

        print(f"  - {show_title} — " + " — ".join(details))

        last_watched = getattr(next_episode, "last_watched_at", None) or getattr(next_episode, "paused_at", None)
        last_watched_text = _format_timestamp(last_watched)
        if last_watched_text:
            print(f"    Last watched: {last_watched_text}")

        additional = [episode for episode in tracked_episodes if not _episode_matches(episode, next_episode)]
        if additional:
            print("    Additional episodes:")
            seen_keys: set[tuple[Any, ...]] = set()
            for extra in additional:
                key = _episode_key(extra)
                if key in seen_keys:
                    continue
                seen_keys.add(key)
                extra_code = _format_episode_code(getattr(extra, "season", None), getattr(extra, "number", None))
                extra_media = getattr(extra, "episode", None)
                extra_title = getattr(extra_media, "title", None) or "Unknown Episode"
                extra_progress = _format_progress(
                    getattr(extra, "progress", None),
                    completed=getattr(extra, "completed", False),
                )
                extra_resume = " (resume available)" if getattr(extra, "resume_available", False) else ""
                completed_suffix = " (completed)" if getattr(extra, "completed", False) else ""
                print(
                    f"      • {extra_code}: {extra_title} — Progress: {extra_progress}{extra_resume}{completed_suffix}"
                )
...
def _format_remaining(count: Optional[int]) -> Optional[str]:
    if count is None or count <= 0:
        return None
    label = "episode" if count == 1 else "episodes"
    return f"{count} {label} remaining"


def _episode_key(episode: Any) -> tuple[Any, ...]:
    episode_media = getattr(episode, "episode", None)
    title = getattr(episode_media, "title", None)
    return (
        getattr(episode, "playback_id", None),
        getattr(episode, "season", None),
        getattr(episode, "number", None),
        title,
    )


def _episode_matches(left: Any, right: Any) -> bool:
    if left is right:
        return True
    if left is None or right is None:
        return False

    left_playback = getattr(left, "playback_id", None)
    right_playback = getattr(right, "playback_id", None)
    if left_playback is not None and right_playback is not None and left_playback == right_playback:
        return True

    left_season = getattr(left, "season", None)
    right_season = getattr(right, "season", None)
    left_number = getattr(left, "number", None)
    right_number = getattr(right, "number", None)
    if (left_season is not None or left_number is not None) and left_season == right_season and left_number == right_number:
        return True

    left_media = getattr(left, "episode", None)
    right_media = getattr(right, "episode", None)
    left_title = getattr(left_media, "title", None)
    right_title = getattr(right_media, "title", None)
    if left_title and right_title and left_title == right_title and left_season == right_season and left_number == right_number:
        return True
